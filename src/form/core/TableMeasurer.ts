...
export class TableMeasurer<T> {
  ...
  private getTableHeight(tableData: TableData<T>, config: TableConfig): number {
    ...
    if (config.isShowXSequence()) {
      topHeight = DrawUtils.getTextHeight(paint, config.getXSequenceStyle()) + 2 * config.getSequenceVerticalPadding();
    }
    ...
    if (tableData.isShowCount()) {
      const countHeight = DrawUtils.getTextHeight(paint, config.getCountStyle()) + 2 * config.getVerticalPadding();
  ...
  private getTableWidth(tableData: TableData<T>, config: TableConfig): number {
    ...
    if (config.isShowYSequence()) {
      const yAxisWidth =
        paint.measureText(tableData.getYSequenceFormat().format(totalSize)) + 2 * config.getSequenceHorizontalPadding();
    ...
    for (const column of tableData.getChildColumns()) {
      const columnNameWidth =
        tableData.getTitleDrawFormat().measureWidth(column, config) + config.getColumnTitleHorizontalPadding() * 2;
      let columnWidth = 0;
      ...
      for (const range of column.getRanges()) {
        if (range && range.length === 2) {
          if (range[0] <= position && range[1] >= position) {
            height =
              (column.getDrawFormat().measureHeight(column, range[0], config) + 2 * config.getVerticalPadding()) /
              (range[1] - range[0] + 1);
          }
        }
      }
