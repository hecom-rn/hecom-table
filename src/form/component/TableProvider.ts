...
export class TableProvider<T> implements TableClickObserver {
  ...
  private drawTitle(canvas: Canvas): void {
    ...
    if (this.config.getColumnTitleBackground()) {
      this.tempRect.set(this.showRect.left, this.showRect.top, this.showRect.right, this.showRect.top + clipHeight);
  ...
  private fillColumnTitle(canvas: Canvas, info: ColumnInfo, left: number): void {
    let top = info.top * this.config.getZoom() + (this.config.isFixedTitle() ? this.showRect.top : this.scaleRect.top);
    ...
    if (DrawUtils.isMixRect(this.showRect, left, top, right, bottom)) {
      ...
      if (this.config.getTableGridFormat()) {
        ...
        this.config.getTableGridFormat().drawColumnTitleGrid(canvas, this.tempRect, info.column, position, paint);
      }
