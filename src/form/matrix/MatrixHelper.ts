...
class Scroller {
  constructor(_context: SmartTable<any>) {}

  setFinalX(_number: number) {}

  setFinalY(_number: number) {}
...
class ValueAnimator {
  static ofObject(_evaluator: PointEvaluator, _startPoint: Point, _endPoint: Point): ValueAnimator {
...
class ScaleGestureDetector {
  constructor(_context: SmartTable<any>, _param2: OnScaleGestureListener) {}
  ...
  implements ITouch, OnScaleGestureListener, SimpleOnGestureListener
{
  ...
  private _isCanZoom: boolean = false;
  ...
  private startFilingAnim(doubleWay: boolean): void {
    ...
    const valueAnimator = ValueAnimator.ofObject(this.evaluator, this.startPoint, this.endPoint);
  ...
  public flingToRow(tableInfo: TableInfo, row: number, offset: number, duration: number): void {
    try {
      ...
      if (
        ...
        this.originalRect.top - this.zoomRect.top < (targetRowHeight - lineHeightArray[row - 1]) * this.getZoom()
      ) {
        return;
      }
