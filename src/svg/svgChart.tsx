      ...
      matches
        ...
        .forEach((match: string) => {
          ...
          if (key) {
            ...
            if (Platform.OS === 'ios' && key === 'font-family' && value === 'sans-serif') {
...
function SvgRoot(props: SVGVEleProps) {
  ...
  return (
    <Svg width={width as string} height={height as string} viewBox={viewBox as string}>
      {children?.map((child) => <SvgEle key={child.key} node={child} />)}
    </Svg>
  );
}

function SvgComponent(props: SVGChartProps, ref: ForwardedRef<(ChartElement & any) | null>) {
  const { onLayout, node, style, handleGesture = true, ...gestureProps } = props;
  ...
  const dispatchEvents = useCallback<DispatchEvents>((types, nativeEvent, eventArgs) => {
  ...
  return svgNode ? (
    <View onLayout={onLayout} testID="component" style={{ ...style, width, height }}>
      <SvgRoot node={svgNode} />
      {handleGesture ? <GestureHandler dispatchEvents={dispatchEvents} {...gestureProps} /> : null}
    </View>
  ) : null;
}
